...
// SumInt32Column calculates the sum of an int32 column in the DirectResult
func (dr *DirectResult) SumInt32Column(colIdx int) (int64, error) {
	values, nulls, err := dr.ExtractInt32Column(colIdx)
	if err != nil {
		return 0, err
	}

	// Manual sum calculation to bypass any potential issues in Int32Sum
	var sum int64
...
// SumInt64Column calculates the sum of an int64 column in the DirectResult
func (dr *DirectResult) SumInt64Column(colIdx int) (int64, error) {
	values, nulls, err := dr.ExtractInt64Column(colIdx)
	if err != nil {
		return 0, err
	}

	// Manual sum calculation to bypass any potential issues in Int64Sum
	var sum int64
	for i, val := range values {
		if !nulls[i] {
			sum += val
...
// SumFloat64Column calculates the sum of a float64 column in the DirectResult
func (dr *DirectResult) SumFloat64Column(colIdx int) (float64, error) {
	values, nulls, err := dr.ExtractFloat64Column(colIdx)
	if err != nil {
		return 0, err
	}

	// Manual sum calculation to bypass any potential issues in Float64Sum
	var sum float64
	for i, val := range values {
		if !nulls[i] {
			sum += val
